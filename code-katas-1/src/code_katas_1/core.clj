...
(defn range
  "Escribir una funcion que cree una lista de enteros en un rango dado.
   Restricciones: range"
  [start end]
  ((fn [list end cont] (if(>= cont end)
                             list
                             (recur (conj list cont) end (+ 1 cont)))) [] end start))
...
(defn zip-map
  ...
  [k v]
   ((fn [mapa k2 v2](if (and (empty? k2) (empty? v2)) mapa 
                             (recur (assoc mapa (first k2) (first v2)) (rest k2) (rest v2))))
